...
fn goto_next(graph: &HashMap<String, Vec<String>>, n: &Vec<String>, single: &mut Vec<String>) -> u32 {
    let mut ret: u32 = 0;
        ...
        match &p[..] {
            ...
            x => {
                ...
                if x.chars().all(|c| matches!(c, 'a'..='z')) {
                    ...
                    ret += goto_next(graph, graph.get(x).unwrap(), &mut single2);
                } else {
                    ret += goto_next(graph, graph.get(x).unwrap(), single);
                }
            },
        }
    }
...
fn main() -> std::io::Result<()> {
    ...
    let result = goto_next(&graph, start, &mut single);
